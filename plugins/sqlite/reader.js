...
Reader.prototype.get = function(table, from, to, what, next) {
  if(what === 'full')
    what = '*';

  this.db.all(`
    SELECT ${what} from ${sqliteUtil.table(table)}
    WHERE start <= ${to} AND start >= ${from}
    ORDER BY start ASC
  `, function(err, rows) {
    if(err) {
...
Reader.prototype.count = function(table, from, to, next) {
  this.db.all(`
    SELECT COUNT(*) as count from ${sqliteUtil.table(table)}
    WHERE start <= ${to} AND start >= ${from}
  `, function(err, res) {
    if(err) {
      console.error(err);
